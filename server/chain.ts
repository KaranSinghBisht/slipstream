/**
 * Backend chain helpers — the proven ER patterns from tests/common.ts, shared
 * by the bridge and the API server. Keys come from .env; never the main wallet.
 */
...
export const WALLET_PATH = process.env.WALLET_KEYPAIR_PATH ?? "./keypairs/burner-devnet.json";
...
/** Pyth Lazer feed accounts delegated into the ER (50–200ms updates). */
export const FEEDS: Record<string, PublicKey> = {
...
type Provider = InstanceType<typeof AnchorProvider>;

export function loadProgram(provider: Provider) {
...
/** Create and fund a throwaway owner so each session gets a fresh vault PDA. */
